...
class TimeSpec extends WordSpec with Matchers {
  ...
  "Convert JSON to time" should {
    "return None for an invalid json" in {
      Json.fromJson[Time](Json.obj()).asOpt shouldBe None
    }
    "return Some wrapping a properly initialized Time for a valid JsonOject" in {
      Json.fromJson[Time](Json.obj("hours" -> 9, "minutes" -> 30)).asOpt shouldEqual Some(Time(9, 30))
    }
  }

  "Convert time to JSON" should {
    "return Some wrapping the original Time" in {
      val time = Time(9, 30)
      Json.toJson[Time](time).asOpt[Time] shouldEqual Some(time)
    }
  }
